...
class Recipe:
    ...
    def to_string(self, num: float = 1) -> str:
        input_string = ", ".join(
            [f"{round(amount, 2)} {inp}" for inp, amount in self.in_per_second(num).items()]
        )
        output_string = ", ".join(
            [f"{round(amount, 2)} {out}" for out, amount in self.out_per_second(num).items()]
        )
...
def get_crafting_speed(category: str) -> float:
    return {"assembling": 0.75, "crafting-with-fluid": 0.75, "smelting": 2}.get(category, 1)

for rec in raw:
    cat = rec.get("category", "assembling")
    sec = rec.get("energy_required", 0.5) * get_crafting_speed(cat)
